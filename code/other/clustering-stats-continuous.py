...
with open('../data/cluster_data-continuous.json', 'r') as file:
...
for i in range(n_clusters):
    ...
    plt.plot(mean_values, label=f'Cluster {i+1} - {labels[i]} ({str(round(hist[i]))} links)' , color=colors[i])
...
plt.yscale('log')
#plt.show()
plt.savefig("Clusters-Metrics-Continuous.pdf", format="pdf", bbox_inches="tight")
sys.exit(0)