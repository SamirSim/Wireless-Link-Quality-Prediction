...
random_receivers = random.sample(node_ids, 1)

nb_parts = 5
Position = range(1,max(random_senders)*nb_parts)
...
for n in random_senders:
    for m in random_receivers:
        ...
        result = split_data(data, nb_parts)

        #size = int(len(data) * 0.33) # First 2/3 of the data are used for training, and the rest is used for testing
        #first, second, third = data[0:size], data[size:2*size], data [2*size:len(data)]
        ...
        for elem in result:
            ...
            ax = fig.add_subplot(len(random_senders),nb_parts,Position[k])
            k = k + 1
