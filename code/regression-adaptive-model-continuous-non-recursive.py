...
def create_sequences(data, window_step):
    X, y = [], []
    for i in range(len(data) - window_step - PREDICTION_WINDOW + 1):
        a = data[i:(i + window_step)]
        X.append(a)
        y.append(data[i + window_step:i + window_step + PREDICTION_WINDOW])
...
for n, m in couples:
    ...
    size = int(len(X) * 0.3) # First 1/3 of the data are used for training, and the rest is used for testing
