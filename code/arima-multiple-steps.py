...
for n, m in couples:
    ...
    q_values = range(2, 8)
    d_values = range(2, 7)
    best_cfg = optimize_model(history, p_values, d_values, q_values) # Find the best configuration (p,d,q) for ARIMA model using grid search
    #print("Best configuration: ", best_cfg, " for ", key)
    #best_cfg = (5,5,5)
    ...
    for _ in range(len(test)//steps):
        try:
            ...
            for t in range(index, index + steps):
                ...
                #print('predicted=%f, expected=%f' % (yhat, obs), false_positive, false_negative, true_positive, true_negative)
                #time.sleep(1)
            index = index + steps
