...
def calculate_ccf(series1, series2):
    return correlate(series1, series2, mode='full')

# Calculate cross-correlation for each pair of time series
n_series = len(time_series_values_padded)
cross_corr_values = np.zeros((n_series, n_series))

for i in range(n_series):
    for j in range(i, n_series):
        ccf = calculate_ccf(time_series_values_padded[i], time_series_values_padded[j])
        cross_corr_values[i, j] = cross_corr_values[j, i] = np.max(ccf)

# Compute the pairwise distance matrix using the cross-correlation values
distance_matrix = squareform(pdist(cross_corr_values, metric='euclidean'))
