...
DEFAULT_FIRMWARE_DURATION=$((1440*60)) # in seconds
BURSTY_FIRMWARE_DURATION=$((60*60)) # in seconds
FIRMWARE="broadcast-example"
SECOND_FIRMWARE="broadcast-example-2"
NODES=$6
#--------------------- DEFINE VARIABLES ---------------------#
...
sed -i "s/#define\ SEND_INTERVAL_SECONDS\ .*/#define\ SEND_INTERVAL_SECONDS\ $2/g" $CODEDIR/$FIRMWARE.c
sed -i "s/#define\ SEND_BUFFER_SIZE\ .*/#define\ SEND_BUFFER_SIZE\ $3/g" $CODEDIR/$FIRMWARE.c
sed -i "s/#define\ NB_PACKETS\ .*/#define\ NB_PACKETS\ $5/g" $CODEDIR/$FIRMWARE.c
#------------------- CONFIGURE FIRMWARE 1 -------------------#
...
sed -i "s/#define\ SEND_INTERVAL_SECONDS\ .*/#define\ SEND_INTERVAL_SECONDS\ $2/g" $CODEDIR/$SECOND_FIRMWARE.c
sed -i "s/#define\ SEND_BUFFER_SIZE\ .*/#define\ SEND_BUFFER_SIZE\ $3/g" $CODEDIR/$SECOND_FIRMWARE.c
sed -i "s/#define\ NB_PACKETS\ .*/#define\ NB_PACKETS\ $7/g" $CODEDIR/$SECOND_FIRMWARE.c
#------------------- CONFIGURE FIRMWARE 2 -------------------#
...
for NODE in "${NODES[@]}"; do
    NODE_LIST="$NODE_LIST -l ${SITE},m3,${NODE},${CODEDIR}/${FIRMWARE}.iotlab-m3"
done
...
iotlab-node --flash $CODEDIR/$FIRMWARE.iotlab-m3 -i $EXPID 
# Wait for contiki
sleep 10 
...
iotlab-node --flash $CODEDIR/$SECOND_FIRMWARE.iotlab-m3 -i $EXPID 
# Wait for contiki
sleep 10
# Run a script for logging and seeding
iotlab-experiment script -i $EXPID --run $SITE,script=serial_script.sh
