...
for n, m in couples:
    ...
    for regressor_string in regressor_strings:
        ...
        elem[regressor_string] = {"window_step": best_window_step, "mae": mae, "mse": mse}
        regressors_results.append(elem)
    best_cfgs[key] = regressors_results

print("best_cfgs: ", best_cfgs)
...
for prediction_step in steps_list:
    ...
    for n, m in couples:
        ...
        # Iterate over each regressor and compare their MSE values
        future_regressor = best_regressor
        print("First best regressor: ", future_regressor)

        for elem in best_cfgs[key]:
            for key, value in elem.items():
                best_regressor = key
                best_window_step = value["window_step"]
                min_mae = value["mae"]
                min_mse = value["mse"]
            ...
            while index < len(test):
                ...
                try:
                    for i in range(index, index+prediction_step):
                        ...
                        if str(regressor) == str(future_regressor):
                            print("Storing prediction: ", predicted_value[0], " from regressor: ", regressor)
                            final_predictions.append(predicted_value[0])
                    ...
                    print(predicted_value[0], test[index+window_step])
                    time.sleep(1)

                    if prediction_step == 1:
                        mse = ((test[index+window_step] - predicted_value[0])**2)/2
                    else:
                        mse = mean_squared_error(predictions, test[index+window_step:index+window_step+prediction_step])

                    print("MSE: ", mse, " min_mse: ", min_mse)
                    
                    if 
                    if mse < min_mse:
                        print("Changed regressor from: ", best_regressor, " to: ", regressor, " with mse: ", mse)
                        future_regressor = regressor
                        min_mse = mse
